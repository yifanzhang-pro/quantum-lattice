...
* After cleanup and \$\mathrm{QFT}*{\mathbb Z*{M\_2}}^{\otimes n}\$ on \$\mathbf Z\$, the measurement outcome \$\mathbf u\$ is **uniform** over
...
* Cleanup per prime \$p\_\eta\$: one reversible inversion \$O((\log p\_\eta)^2)\$ (half-GCD yields \$\tilde O(\log p\_\eta)\$), plus reversible CRT (Garner \$O(\kappa^2)\$ or product-tree \$O(\kappa\log\kappa)\$).
